...
module.exports.run = async (bot, msg, args, prefix) => {
  ...
  if (recipe.data.hits.length === 0) {
    return msg.channel.send(`**Error:** No results found for **${q}**`).catch(e => {
      ...
      message.channel
        ...
        .then(c => {
          ...
          let nEmbed = new Discord.RichEmbed()
            ...
            .setColor(config.colors.white)
            .setAuthor(`${newR.label} | ${newR.source}`, 'https://i.imgur.com/f8rp2PF.png')
            .addField('Yield', rYield, true)
            .addField('Calories', `**${util.commas(~~newR.calories)}**`, true)
            .addField('Total Ingredients', `**${newR.ingredientLines.length}** Ingredients`, true)
          if (newR.totalTime > 0) {
          ...
          message.delete()
          msg.channel.send(nEmbed).catch(e => msg.channel.send(`**Error:** ${e.message}`))
        })
        .catch(e => {
