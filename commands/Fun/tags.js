...
module.exports.run = async (bot, msg, args, prefix) => {
  ...
  const allParams = ['add', 'create', 'edit', 'modify', 'delete', 'remove', 'info', 'owner', 'list']
  ...
  if (modifyParams.includes(param)) {
    ...
    if (['add', 'create'].includes(param)) {
      ...
      return tag.update({ content: tagcontent, lastModified: new Date() }).then(res => {
    ...
    let username = `N/A`
    if ((user = bot.users.get(tag.userID))) username = `${user.username}#${user.discriminator}`
    ...
    if (checktag) {
      ...
      return msg.channel.send(text)
    } else return msg.channel.send(`**Error:** The tag **\`${param}\`** doesn't exist.`)
  }
